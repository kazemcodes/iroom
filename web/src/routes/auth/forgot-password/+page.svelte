...
<div class="min-h-screen flex items-center justify-center px-4 py-12"
	...
	<div class="w-full max-w-md">
		...
		<div class="text-center mb-8">
			...
			<h1 class="text-3xl font-extrabold gradient-text">بازنشانی رمز عبور</h1>
			<p class="text-gray-400 mt-2 font-medium">آی‌روم — کلاس آنلاین هوشمند</p>
		</div>
		...
		<div class="card p-8">
			...
			{#if success && step === 'reset'}
				<div class="mb-5 p-3 bg-green-50 text-green-600 rounded-xl text-sm font-medium flex items-center gap-2 border border-green-100">
					<svg class="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20">
			...
			{#if step === 'email'}
				<form onsubmit={(e) => { e.preventDefault(); handleRequestReset(); }} class="space-y-4">
					<p class="text-sm text-gray-500 font-medium text-center mb-2">
			...
			{:else}
				<form onsubmit={(e) => { e.preventDefault(); handleResetPassword(); }} class="space-y-4">
					...
					<div>
						<label class="block text-sm font-semibold text-gray-700 mb-1.5">تکرار رمز عبور</label>
			...
			<div class="mt-6 text-center">
				<a href="/auth" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
					بازگشت به صفحه ورود
				</a>
			</div>
		</div>
