...
<div class="flex min-h-screen" style="background: var(--sky-bg-dark);">
	<!-- Sidebar -->
		...
		<div class="flex flex-col h-full">
			...
			<div class="px-4 py-5" style="border-bottom: 1px solid var(--sky-border);">
				<div class="flex items-center gap-3">
					<div class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-lg shrink-0"
						style="background: linear-gradient(135deg, #3b82f6, #6366f1); box-shadow: 0 4px 12px rgba(99, 102, 241, 0.3);">
						آ
					</div>
					{#if !sidebarCollapsed}
						<div class="min-w-0">
			...
			<div class="hidden lg:flex justify-center px-3 pt-3">
				<button onclick={() => sidebarCollapsed = !sidebarCollapsed} 
					class="p-2 rounded-lg transition-all duration-200 hover:bg-[var(--sky-bg-input)]"
				...
				{#if $auth.user}
					<div class="flex items-center gap-3 mb-3">
						<div class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0"
							style="background: linear-gradient(135deg, #3b82f6, #8b5cf6); box-shadow: 0 4px 12px rgba(139, 92, 246, 0.3);">
...
<button onclick={() => mobileOpen = true} class="p-2 rounded-lg transition-colors" style="color: var(--sky-text-secondary);" aria-label="منو">
		...
		<header class="sticky top-0 z-10 px-4 py-3 flex items-center justify-between lg:hidden"
			style="background: var(--sky-bg-panel); border-bottom: 1px solid var(--sky-border);">
			<button onclick={() => mobileOpen = true} class="p-2 rounded-lg transition-colors hover:bg-[var(--sky-bg-input)]" style="color: var(--sky-text-secondary);" aria-label="منو">
				<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
				</svg>
			</button>
			...
			<button class="notification-bell p-2 rounded-lg transition-colors hover:bg-[var(--sky-bg-input)] relative"
				style="color: var(--sky-text-secondary);"
				onclick={() => showNotifications = !showNotifications}>
				<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
					<path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
...
<style>
	...
	:global(.sky-sidebar-link.active) {
		...
		color: white;
		font-weight: 600;
		box-shadow: inset 3px 0 0 0 #6366f1, 0 4px 12px rgba(59, 130, 246, 0.3);
	}
	...
	:global(.sky-sidebar-link.active::before) {
		...
		color: var(--sky-accent-red) !important;
	}
</style>
