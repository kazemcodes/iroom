			...
			<nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
				...
				{#if !sidebarCollapsed}
					...
					{#if !sidebarCollapsed && item.href === '/classes' && counts.classes > 0}
						<span class="text-[10px] px-1.5 py-0.5 bg-gray-200 text-gray-600 rounded-full ms-auto">{counts.classes}</span>
					{:else if !sidebarCollapsed && item.href === '/sessions' && counts.sessions > 0}
						<span class="text-[10px] px-1.5 py-0.5 bg-gray-200 text-gray-600 rounded-full ms-auto">{counts.sessions}</span>
					{/if}
					</a>
				{/each}
