				...
				<div style="background:var(--color-pure);border:1px solid var(--color-zen-garden);border-radius:12px;padding:20px;transition:box-shadow 0.2s;">
					<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;">
						<div style="display:flex;align-items:center;gap:12px;">
							<div style="position:relative;">
								<div style="width:48px;height:48px;border-radius:12px;display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:18px;background:{room.color || 'var(--color-crystal-clear)'};">
									{room.name.charAt(0)}
								</div>
							</div>
							<div>
								<h3 style="font-weight:700;color:var(--color-midnight-sky);">{room.name}</h3>
								<p style="font-size:0.75rem;color:var(--color-mystic-sea);margin-top:2px;">{formatDate(room.created_at)}</p>
							</div>
						</div>
						<span style="font-size:0.75rem;padding:4px 10px;border-radius:20px;font-weight:600;background:{room.guest_login_enabled ? 'rgba(64,191,127,0.1)' : 'var(--color-secret-glow)'};color:{room.guest_login_enabled ? 'var(--color-lush-meadow)' : 'var(--color-moonlit-mist)'};">
							{room.guest_login_enabled ? 'مهمان فعال' : 'مهمان غیرفعال'}
						</span>
					</div>
					...
					<div style="display:flex;align-items:center;gap:16px;font-size:0.75rem;color:var(--color-mystic-sea);margin-bottom:16px;">
						<div style="display:flex;align-items:center;gap:6px;">
							...
							<span>{toPersian(userCount)} کاربر</span>
						</div>
					</div>

					{#if room.description}
						<p class="text-xs mb-3" style="color: var(--color-mystic-sea);">{room.description}</p>
					{/if}

					<div class="flex items-center gap-2">
						<a href="/room/{room.slug}" target="_blank" class="sky-btn flex-1" style="background: rgba(64,191,127,0.12); color: var(--color-lush-meadow); font-size: 12px; padding: 0.45rem;">ورود</a>
						<a href="/admin/rooms/{room.id}/users" class="sky-btn flex-1" style="background: rgba(35,185,215,0.12); color: var(--color-crystal-clear); font-size: 12px; padding: 0.45rem;">کاربران</a>
						<button onclick={() => copyLink(room.slug)} class="sky-btn sky-btn-secondary flex-1" style="font-size: 12px; padding: 0.45rem;">کپی لینک</button>
						<button onclick={() => confirmDeleteRoom(room.id)} class="sky-btn-icon" style="width:34px;height:34px;" title="حذف">
